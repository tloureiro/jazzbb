...
- **Keyboard shortcuts**: `Ctrl/Cmd+N` new note, `Ctrl/Cmd+S` save, `Ctrl/Cmd+P` search, `Ctrl/Cmd+K` opens the command palette, `Ctrl/Cmd+Shift+H` collapses the top bar, `Ctrl/Cmd+Shift+B` toggles the vault sidebar, `Ctrl/Cmd+Alt+K` folds the current heading, `Ctrl/Cmd+Alt+F` toggles the frontmatter editor/panel, `Esc` closes overlays. Sidebar buttons mirror these actions, and every binding can be remapped from the shortcut modal (Cmd/Ctrl + /) by clicking the shortcut chip; overrides live in local storage per browser profile.
...
- **Frontmatter**: YAML frontmatter stays hidden in the TipTap surface; a badge appears next to the note title when it exists. Toggle the inline editor via the palette or `Ctrl/Cmd+Alt+F` to edit it with syntax highlighting and surface a read-only panel in the right column with structured metadata.
